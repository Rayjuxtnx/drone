...
import { BatteryFull, BatteryLow, Wrench, Plane } from 'lucide-react';
...
const statusConfig: { [key in DroneStatus]: { icon: React.ElementType, color: string } } = {
  ...
  Offline: { icon: Plane, color: 'text-gray-500' },
};
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div className="p-2 bg-muted rounded-full mr-4">
                                            <drone.status.icon className={cn("h-5 w-5", statusConfig[drone.status].color)} />
                                        </div>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                        <p>{drone.status}</p>
