...
function TrackingPageContent() {
  ...
  useEffect(() => {
    if (mission?.status !== 'In Progress' || !mission.telemetry) {
      return;
    }

    const interval = setInterval(() => {
      setMission(prevMission => {
        if (!prevMission || !prevMission.telemetry) return prevMission;

        const newTelemetry = {
          ...prevMission.telemetry,
          lat: prevMission.telemetry.location.lat + (Math.random() - 0.5) * 0.001,
          lng: prevMission.telemetry.location.lng + (Math.random() - 0.5) * 0.001,
          battery: Math.max(0, prevMission.telemetry.battery - 1),
        };
        ...
        const newMissionState = {
            ...
            telemetry: {
                ...newTelemetry,
                location: {
                    lat: newTelemetry.lat,
                    lng: newTelemetry.lng
                }
            }
        };

        // Update the mission in the global state as well
        setMissions(prevMissions => 
          prevMissions.map(m => m.id === missionId ? newMissionState : m)
        );
  ...
  }, [mission?.status, missionId, setMissions]);
