...
import { CalendarIcon, MapPin, DollarSign, Book, LocateFixed, Loader2, Rocket } from "lucide-react";
...
export default function BookingPage() {
    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // A real app would use a reverse geocoding service. Here, we'll use a Kenyan city.
          const locationString = `Nairobi, Kenya (${latitude.toFixed(4)}, ${longitude.toFixed(4)})`;
  ...
  function onSubmit(data: BookingFormValues) {
    const newMission: Mission = {
      ...
      estimatedPrice: Math.floor(Math.random() * 15000) + 5000,
      ...data,
    ...
    toast({
      ...
      description: `Your mission #${newMission.id} is pending admin approval.`,
    });
      ...
      <div className="container py-12">
        <Card className="max-w-3xl mx-auto">
          <CardHeader>
            ...
            <CardDescription>Fill out the details below to schedule your drone service in Kenya.</CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    />
                     <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                           <FormLabel className="flex items-center gap-2"><Rocket className="h-4 w-4"/> Drone Model</FormLabel>
                          <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl>
                              <SelectTrigger>
                                <SelectValue placeholder="Select a drone model..." />
                              </SelectTrigger>
                            </FormControl>
                            <SelectContent>
                              {drones.map(drone => (
                                <SelectItem key={drone.id} value={drone.model}>{drone.model}</SelectItem>
                              ))}
                            </SelectContent>
                          </Select>
                          <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="e.g., Nyali, Mombasa" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
